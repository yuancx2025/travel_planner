...
class ResearchAgent:
    ...
    def _concurrency_limit(self) -> int:
        return config.RESEARCH_MAX_CONCURRENCY
    ...
    def research(
        ...
        if not destination:
            # Return empty ResearchOutput with error indication
            return ResearchOutput(raw={"error": "destination_city is required"})
    ...
    async def _research_async(
        ...
        # Convert results dict to ResearchOutput schema
        return self._convert_to_research_output(results)
    
    def _convert_to_research_output(self, results: Dict[str, Any]) -> ResearchOutput:
        """Convert raw results dict to ResearchOutput schema."""
        # Convert attractions
        attractions_list: List[Attraction] = []
        for attr_dict in results.get("attractions", []):
            if isinstance(attr_dict, dict):
                coord_dict = attr_dict.get("coord")
                coord = None
                if coord_dict:
                    coord = Coordinate(
                ...
                attractions_list.append(Attraction(
                    ...
                    coord=coord,
                    category=attr_dict.get("category"),
                    rating=attr_dict.get("rating"),
                    review_count=attr_dict.get("review_count"),
                    source=attr_dict.get("source"),
                    error=attr_dict.get("error"),
                    raw=attr_dict,
                ))
        
        # Convert dining/restaurants
        dining_list: List[Restaurant] = []
        for rest_dict in results.get("dining", []):
            if isinstance(rest_dict, dict):
                coord_dict = rest_dict.get("coord")
                coord = None
                if coord_dict:
                    coord = Coordinate(
                        lat=coord_dict.get("lat"),
                ...
                dining_list.append(Restaurant(
                    id=rest_dict.get("id"),
                    name=rest_dict.get("name"),
                    address=rest_dict.get("address"),
                    coord=coord,
                    rating=rest_dict.get("rating"),
                    review_count=rest_dict.get("review_count"),
                    price_level=rest_dict.get("price_level"),
                    source=rest_dict.get("source"),
                    error=rest_dict.get("error"),
                    raw=rest_dict,
                ))
        
        # Convert hotels
        hotels_list: List[Hotel] = []
        for hotel_dict in results.get("hotels", []):
            if isinstance(hotel_dict, dict):
                hotels_list.append(Hotel(
                    hotel_id=hotel_dict.get("hotel_id"),
                    name=hotel_dict.get("name"),
                    address=hotel_dict.get("address"),
                    price=self._price_to_float(hotel_dict.get("price")),
                    currency=hotel_dict.get("currency"),
                    rating=hotel_dict.get("rating"),
                    source=hotel_dict.get("source"),
                    error=hotel_dict.get("error"),
                    raw=hotel_dict,
                ))
        
        # Convert weather (handle string values with units)
        weather_list: List[WeatherDay] = []
        for weather_dict in results.get("weather", []):
            if isinstance(weather_dict, dict):
                try:
                    # Parse temperature strings like "68 °F" or "20 °C"
                    def parse_temp(value: Any) -> Optional[float]:
                        if value is None:
                            return None
                        if isinstance(value, (int, float)):
                            return float(value)
                        if isinstance(value, str):
                            # Extract number from strings like "68 °F" or "20.5°C"
                            import re
                    ...
                    # Parse precipitation strings like "0.0 in" or "5 mm"
                    def parse_precip(value: Any) -> Optional[float]:
                        if value is None:
                            return None
                        if isinstance(value, (int, float)):
                            return float(value)
                    ...
                    weather_list.append(WeatherDay(
                        ...
                        temp_max=parse_temp(weather_dict.get("temp_max") or weather_dict.get("temp_high")),
                        temp_min=parse_temp(weather_dict.get("temp_min") or weather_dict.get("temp_low")),
                        temp_high=parse_temp(weather_dict.get("temp_high")),
                        temp_low=parse_temp(weather_dict.get("temp_low")),
                        precip=parse_precip(weather_dict.get("precip")),
                        precipitation=parse_precip(weather_dict.get("precipitation")),
                        conditions=weather_dict.get("conditions"),
                        summary=weather_dict.get("summary"),
                        raw=weather_dict,
                    ))
                except Exception as e:
                    # If weather conversion fails, skip it but don't fail entire research
                    # Add error entry instead
                    weather_list.append(WeatherDay(
                        date=weather_dict.get("date"),
                        error=f"Weather parsing failed: {e}",
                        raw=weather_dict,
                    ))
        ...
        try:
            research_output = ResearchOutput(
                ...
                car_rentals=results.get("car_rentals", []),
                fuel_prices=results.get("fuel_prices"),
                distances=results.get("distances", []),
                raw=results,
            )
            return research_output
        except Exception as e:
            # If validation fails, return ResearchOutput with error but preserve attractions/dining
            # Create output with error but preserve successful data
            results_with_error = dict(results)
            results_with_error["error"] = f"Validation error: {e}"
            return ResearchOutput(
                attractions=attractions_list,
                dining=dining_list,
                hotels=hotels_list,
                weather=[],  # Skip weather if it causes validation errors
                flights=results.get("flights", []),
                car_rentals=results.get("car_rentals", []),
                fuel_prices=results.get("fuel_prices"),
                distances=results.get("distances", []),
                raw=results_with_error,
            )
    
    def _price_to_float(self, price: Any) -> Optional[float]:
    ...
    def _get_attractions(
        ...
        try:
            ...
            result = self._prioritize_results(base_results, priority_names, fetch_fn)
            return result
        except Exception as e:
