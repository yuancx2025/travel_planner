...
from workflows.schemas import ItineraryOutput, DaySchedule, Stop, Route, Coordinate, CriticEvaluation
...
class ItineraryAgent:
    ...
    def __init__(
        self,
        *,
        default_blocks_per_day: int = None,
        model_name: str = None,
        temperature: float = None,
        llm: Optional[ChatGoogleGenerativeAI] = None,
    ) -> None:
        self.default_blocks_per_day = max(1, default_blocks_per_day if default_blocks_per_day is not None else config.ITINERARY_DEFAULT_BLOCKS_PER_DAY)
    ...
    def build_itinerary(
        ...
        feedback: Optional[CriticEvaluation] = None,
    ) -> ItineraryOutput:
        coroutine = self._build_itinerary_async(preferences, attractions, research, feedback)
        try:
    ...
    async def _build_itinerary_async(
        ...
        # Convert day_blocks to DaySchedule objects
        day_schedules: List[DaySchedule] = []
        for day_block in day_blocks:
            stops_list: List[Stop] = []
            for stop_dict in day_block.get("stops", []):
                coord_dict = stop_dict.get("coord")
                coord = None
                if coord_dict:
                    coord = Coordinate(
                ...
                stops_list.append(Stop(
                    ...
                    address=stop_dict.get("address"),
                    coord=coord,
                    start_time=stop_dict.get("start_time"),
                    duration_hours=stop_dict.get("duration_hours"),
                    streetview_url=stop_dict.get("streetview_url"),
                    category=stop_dict.get("category"),
                    rating=stop_dict.get("rating"),
                    raw=stop_dict,
                ))
            
            route_dict = day_block.get("route")
            route = None
            if route_dict:
                route = Route(
                    distance_m=route_dict.get("distance_m"),
            ...
            day_schedules.append(DaySchedule(
                day=day_block.get("day", len(day_schedules) + 1),
        ...
        itinerary_meta = meta.copy() if meta else {}
        if self._llm_error:
            itinerary_meta["llm_error"] = self._llm_error
        
        return ItineraryOutput(
            days=day_schedules,
            meta=itinerary_meta if itinerary_meta else None,
        )
    ...
    def _plan_day_blocks(
        ...
        # Apply feedback adjustments to preferences/attractions if feedback exists
        if feedback and not feedback.requirements_met:
            adjusted_preferences, adjusted_attractions = self._apply_feedback_adjustments(
                preferences, attractions, feedback
            )
            # Re-preprocess with adjusted inputs
            preprocessed = self._preprocess_inputs(adjusted_preferences, adjusted_attractions, research)
            meta["feedback_applied"] = True
            meta["feedback_iteration"] = True

        if preprocessed["activities"]:
            schedule_result = self._generate_llm_schedule(preferences, research, preprocessed, feedback)
        else:
        ...
        fallback_blocks = self._chunk_attractions(preferences, attractions, feedback)
    ...
    def _generate_llm_schedule(
        ...
        # Extract parameters for the prompt template
        total_days = preprocessed.get("travel_days", 1)
        traveler_profile = preprocessed.get("preferences_summary", "Not specified")
        day_constraints = preprocessed.get("day_constraints", {})
        constraints = (
            f"Day starts at {day_constraints.get('day_start', '09:00')}, "
            f"ends at {day_constraints.get('day_end', '21:30')}. "
            f"Target {day_constraints.get('blocks_per_day', 3)} activities per day."
        )
        
        # Add feedback constraints if available
        if feedback and not feedback.requirements_met:
            feedback_constraints = self._format_feedback_for_prompt(feedback)
            constraints += f"\n\nIMPORTANT: Previous iteration feedback:\n{feedback_constraints}"
            constraints += "\nPlease adjust the itinerary to address these issues."
        ...
        user_msg = HumanMessage(content=self._render_llm_payload(preferences, research, preprocessed, feedback))
    ...
    def _normalize_attraction(
        ...
        return _Activity(
            ...
            duration_hours=self._estimate_duration(category),
    ...
    def _apply_feedback_adjustments(
        ...
        preferences: Dict[str, Any],
        attractions: List[Dict[str, Any]],
        feedback: CriticEvaluation,
    ) -> Tuple[Dict[str, Any], List[Dict[str, Any]]]:
        """
        Apply feedback adjustments to preferences and attractions.
        Returns adjusted preferences and attractions.
        """
        adjusted_preferences = preferences.copy()
        adjusted_attractions = attractions.copy()
        
        # If budget exceeded, reduce activity count
        if feedback.budget_violation:
            reduction_factor = 0.75  # Reduce by 25%
            target_count = max(1, int(len(attractions) * reduction_factor))
            adjusted_attractions = attractions[:target_count]
        
        # If activity preferences not met, prioritize matching activities
        activity_pref_violations = [
            v for v in feedback.violations 
            if v.requirement == "activity_preferences"
        ]
        if activity_pref_violations:
            # Try to prioritize attractions that match activity preferences
            # This is a simple heuristic - could be enhanced
            pass
        
        # If travel days mismatch, adjust
        travel_days_violations = [
            v for v in feedback.violations 
            if v.requirement == "travel_days"
        ]
        if travel_days_violations:
            # The violation reason contains the expected days
            # We'll let the LLM handle this through the prompt
            pass
        
        return adjusted_preferences, adjusted_attractions
    
    def _format_feedback_for_prompt(self, feedback: CriticEvaluation) -> str:
        """Format feedback evaluation for inclusion in LLM prompt."""
        lines = []
        
        if feedback.failed_requirements:
            lines.append("Issues found in previous iteration:")
            for req in feedback.failed_requirements:
                lines.append(f"  - {req}")
        
        if feedback.budget_violation:
            lines.append(f"\nBudget constraint: Exceeded by ${feedback.budget_violation.overage:.2f} "
                        f"({feedback.budget_violation.percentage_over:.1f}%). "
                        f"Expected: ${feedback.budget_violation.expected_budget:.2f}, "
                        f"Actual: ${feedback.budget_violation.actual_cost:.2f}")
    ...
    def _chunk_attractions(
        ...
        # Apply feedback: reduce activities if budget exceeded
        filtered_attractions = attractions
        if feedback and feedback.budget_violation:
            # Reduce number of attractions if budget is exceeded
            reduction_factor = 0.7  # Reduce by 30%
            target_count = max(1, int(len(attractions) * reduction_factor))
            filtered_attractions = attractions[:target_count]
        
        total_days = max(1, self._safe_int(preferences.get("travel_days"), len(filtered_attractions) or 1))
        per_day = max(1, math.ceil(max(1, len(filtered_attractions)) / total_days))
        blocks: List[Dict[str, Any]] = []
        start_hour = 9
        block_hours = max(2, int(12 / self.default_blocks_per_day))

        for idx in range(total_days):
            chunk = filtered_attractions[idx * per_day : (idx + 1) * per_day]
            stops: List[Dict[str, Any]] = []
    ...
    def _parse_hours(self, hours: Optional[Iterable[str]]) -> Dict[str, Dict[str, Optional[str]]]:
        ...
        time_pattern = re.compile(r"(?P<hour>\d{1,2})(?::(?P<minute>\d{2}))?\s*(?P<ampm>[AaPp][Mm])?")
        ...
        for entry in hours:
            ...
            day, rest = match.group("day").lower(), match.group("rest")
            ...
            if len(times) >= 2:
                open_min = self._convert_time_tuple(times[0])
                close_min = self._convert_time_tuple(times[-1])
                parsed[day] = {
                    "open": self._format_minutes(open_min) if open_min is not None else None,
                    "close": self._format_minutes(close_min) if close_min is not None else None,
                }

        return parsed

    def _convert_time_tuple(self, match: Tuple[str, str, str]) -> Optional[int]:
        """Convert (hour, minute, ampm) tuple to minutes since midnight."""
        if not match:
            return None
        hour, minute, ampm = int(match[0]), int(match[1] or 0), (match[2] or "").lower()
    ...
    def _estimate_duration(self, category: Optional[str]) -> float:
        ...
        duration_map = [
            (("theme_park", "zoo"), 3.5),
            (("museum", "gallery"), 2.5),
            (("park", "garden", "trail"), 2.0),
            (("tour", "sight", "viewpoint", "tower"), 1.5),
            (("shopping", "market"), 1.5),
        ]
        for keywords, duration in duration_map:
    ...
    def _derive_ideal_window(
        ...
        if category:
            cat_lower = category.lower()
            if any(kw in cat_lower for kw in ("observatory", "viewpoint")):
                return "sunset"
            if any(kw in cat_lower for kw in ("night", "bar")):
                return "evening"
        return "afternoon"
