...
import config
from agents.prompts import PromptTemplate, load_prompt_template
...
class ChatAgent:
    ...
    def __init__(
        ...
        guidance_summary_prompt: Optional[PromptTemplate] = None,
        guidance_ask_prompt: Optional[PromptTemplate] = None,
    ):
        # Validate API key
        ...
        model_name = model_name if model_name is not None else config.DEFAULT_MODEL_NAME
        ...
        self.guidance_summary_prompt = guidance_summary_prompt or load_prompt_template("chat_guidance_summary", "chat_guidance_summary.md")
        self.guidance_ask_prompt = guidance_ask_prompt or load_prompt_template("chat_guidance_ask", "chat_guidance_ask.md")
    ...
    def _build_guidance_system_message(self, state: Dict[str, Any], missing: List[str]) -> SystemMessage:
        ...
        if not missing:
            ...
            def _get(k, default="(not provided)"): return state.get(k) or default
            kids_str = _get("kids", "no")
            car_str = _get("need_car_rental", "no")
            pref_attractions = state.get("preferred_attractions") or []
            pref_restaurants = state.get("preferred_restaurants") or []
            ...
            text = self.guidance_summary_prompt.format(
                name=_get('name'),
                destination_city=_get('destination_city'),
                travel_days=_get('travel_days'),
                start_date=_get('start_date'),
                budget_usd=_get('budget_usd'),
                num_people=_get('num_people'),
                kids=kids_str,
                activity_pref=_get('activity_pref'),
                need_car_rental=car_str,
                hotel_room_pref=_get('hotel_room_pref'),
                cuisine_pref=_get('cuisine_pref'),
                preferred_attractions=pref_attr_str,
                preferred_restaurants=pref_dine_str,
            )
        else:
            # Ask for the next missing field(s) naturally (1–2 at a time)
            ask_these = missing[:2]
            asks = [labels[m] for m in ask_these if m in labels]
            text = self.guidance_ask_prompt.format(
                current_state_json=json.dumps(state, ensure_ascii=False),
                missing_fields=', '.join(missing),
                missing_field_labels=' and '.join(asks),
            )
